...
class HankelConv(nn.Module):
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        ...
        # Clamp modes to input dimensions if they are smaller
        eff_modes_r = min(dim_r, modes_r)
        eff_modes_theta = min(dim_theta // 2 + 1, modes_theta)
        ...
        # 4. Create output tensor in frequency domain
        ...
        slices_w = [slice(None)] * 4 # (in, out, r, theta)
        
        slices_in[self.r_axis] = slice(None, eff_modes_r)
        slices_in[self.theta_axis] = slice(None, eff_modes_theta)
        
        slices_out[self.r_axis] = slice(None, eff_modes_r)
        slices_out[self.theta_axis] = slice(None, eff_modes_theta)
        
        slices_w[2] = slice(None, eff_modes_r)
        slices_w[3] = slice(None, eff_modes_theta)
        
        x_corner = x_ht[tuple(slices_in)]
        w_corner = w[tuple(slices_w)]
