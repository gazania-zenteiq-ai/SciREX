...
Config-driven variant of ``train_darcy2d_wno.py`` with no argparse surface.
Override settings via ``DarcyMatWNO2DConfig`` or environment variables.
"""
...
def apply_env_overrides(config: DarcyMatWNO2DConfig) -> DarcyMatWNO2DConfig:
    """Apply lightweight environment-variable overrides, matching repo style."""
    ...
    config.subsample_rate = int(
        os.environ.get("DARCY_SUBSAMPLE_RATE", config.subsample_rate)
    )
    config.n_train = int(os.environ.get("DARCY_N_TRAIN", config.n_train))
    config.n_test = int(os.environ.get("DARCY_N_TEST", config.n_test))
    config.batch_size = int(os.environ.get("DARCY_BATCH_SIZE", config.batch_size))
    config.epochs = int(os.environ.get("DARCY_EPOCHS", config.epochs))
    config.learning_rate = float(
        os.environ.get("DARCY_LEARNING_RATE", config.learning_rate)
    )
    config.weight_decay = float(
        os.environ.get("DARCY_WEIGHT_DECAY", config.weight_decay)
    )
    config.hidden_channels = int(
        os.environ.get("DARCY_HIDDEN_CHANNELS", config.hidden_channels)
    )
    config.n_layers = int(os.environ.get("DARCY_N_LAYERS", config.n_layers))
    config.level = int(os.environ.get("DARCY_LEVEL", config.level))
    config.wavelet = os.environ.get("DARCY_WAVELET", config.wavelet)
    config.mode = os.environ.get("DARCY_MODE", config.mode)
    config.padding = float(os.environ.get("DARCY_PADDING", config.padding))
    config.lifting_channel_ratio = int(
        os.environ.get(
            "DARCY_LIFTING_CHANNEL_RATIO",
            config.lifting_channel_ratio,
        )
    )
    config.projection_channel_ratio = int(
        os.environ.get(
            "DARCY_PROJECTION_CHANNEL_RATIO",
            config.projection_channel_ratio,
        )
    )
    config.skip_type = os.environ.get("DARCY_SKIP_TYPE", config.skip_type)
    config.encode_input = os.environ.get(
        "DARCY_ENCODE_INPUT",
        str(config.encode_input),
    ).lower() in {"1", "true", "yes", "on"}
    config.encode_output = os.environ.get(
        "DARCY_ENCODE_OUTPUT",
        str(config.encode_output),
    ).lower() in {"1", "true", "yes", "on"}
    config.seed = int(os.environ.get("DARCY_SEED", config.seed))
    config.run_name = os.environ.get("DARCY_RUN_NAME", "darcy2d_wno_config")
    return config
...
def main() -> None:
    config = apply_env_overrides(DarcyMatWNO2DConfig())
